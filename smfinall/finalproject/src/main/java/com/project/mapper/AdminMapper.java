...
public interface AdminMapper {
 ...
 // 🔹 관리자 게시물 목록 조회 (수정)
    ...
    /** ✅ 2. 특정 레시피 조회 */
    @Select("SELECT r.*, i.ingredient_id, i.name AS ingredient_name " +
            "FROM Recipes r " +
    ...
    @Results({
        ...
        @Result(property = "ingredients", column = "recipes_id",
                many = @Many(select = "getIngredientsByRecipeId"))
    })
    ...
    @Update("UPDATE Recipes SET foodName = #{foodName}, foodImg = #{foodImg}, " +
            ...
            "stepImg1 = CASE WHEN #{step1} IS NULL THEN NULL ELSE #{stepImg1} END, " +
            "stepImg2 = CASE WHEN #{step2} IS NULL THEN NULL ELSE #{stepImg2} END, " +
            "stepImg3 = CASE WHEN #{step3} IS NULL THEN NULL ELSE #{stepImg3} END, " +
            "stepImg4 = CASE WHEN #{step4} IS NULL THEN NULL ELSE #{stepImg4} END, " +
            "stepImg5 = CASE WHEN #{step5} IS NULL THEN NULL ELSE #{stepImg5} END, " +
            "stepImg6 = CASE WHEN #{step6} IS NULL THEN NULL ELSE #{stepImg6} END, " +
            "weatherId = #{weatherId} " +
