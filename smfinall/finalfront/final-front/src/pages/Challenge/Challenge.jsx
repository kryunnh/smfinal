        ...
        <div className="recipes-main">
            ...
            <button className="speaker" onClick={ ()=>{if(!category){alert("카테고리를 선택해 주세요."); return;} isRecording ? stopRecording() : startRecording()}} disabled={false}>
                ...
                <input
                    ...
                    disabled={!category}
                />
                <button className="rc-se" onClick={ ()=>{if(!category){alert("카테고리를 선택해 주세요."); return;} handleSearch()}} disabled={false}>검색</button>
            </div>
            ...
            <div className="recipes-grid">
                {recipes.slice(0, visibleCount).map((recipe) => (
                    ...
                    >
                        <Link to={`/challenge/${recipe.userRecipesId}` } onClick={()=> { handleClick(recipe.userRecipesId)}}>
