...
const InquiryList = () => {
  ...
  return (
    <div className="inquiry-list-container">
      <div className="inquiry-header">1:1 문의</div>
          ...
          {inquiries.length > 0 ? (
            inquiries.map((inquiry) => (
              <tr key={inquiry.id}>
                ...
                <td className={`reply-status ${!inquiry.reply ? "no-reply" : ""}`}>
  {inquiry.reply ? "답변" : "미답변"}
</td>
                <td>
                  <button className="delete-button" onClick={() => handleDelete(inquiry.id)}>
                    삭제
                  </button>
                </td> {/* ✅ 삭제 버튼 추가 */}
              </tr>
      ...
      {/* ✅ 문의하기 버튼 추가 */}
      <button className="inquiry-button" onClick={() => navigate("/mypage/inquiry/new")}>
        문의하기
      </button>
    </div>
  );
