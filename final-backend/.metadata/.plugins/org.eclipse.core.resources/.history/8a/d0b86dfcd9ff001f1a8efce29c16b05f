...
public interface ReviewMapper {
	...
	@Select("SELECT r.*, u.name, u.id FROM Reviews r JOIN Users u ON r.user_id = u.id WHERE r.recipes_id = #{id} ORDER BY r.created_at DESC")
	...
	@Update("UPDATE Reviews SET review_text = #{reviewText}, rating = #{rating}, updated_at = NOW() WHERE id = #{id}")
