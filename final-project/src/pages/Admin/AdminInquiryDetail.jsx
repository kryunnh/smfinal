  ...
  return (
    <div className="inquiry-detail-container">
      ...
      <div className="inquiry-header">
        ...
        <p><strong>이메일:</strong> {inquiry.userEmail}</p>
        <p><strong>작성일:</strong> {new Date(inquiry.createdAt).toLocaleDateString()}</p>
        <p><strong>상태:</strong> {inquiry.reply ? "답변 완료" : "대기 중"}</p>
      </div>
      ...
      <button className="back-btn" onClick={() => navigate(-1)}>🔙 이전으로</button>
        <button className="save-btn" onClick={handleReplySubmit}>답변 저장</button>
        <button className="delete-reply-btn" onClick={() => setReply("")}>답변 삭제</button>
        <button className="delete-inquiry-btn" onClick={handleDeleteInquiry}>문의 삭제</button>
      </div>
    </div>
  );
};
